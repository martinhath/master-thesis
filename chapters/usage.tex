...
\fixme{29/05 08:36 Write more here}
...
\code{List} are very similar to that of the \code{Stack} and \code{Queue}, and is therefore
omitted.
...
the \code{.next} field in a node for a \emph{tag}\footnote{Now we finally understand why CMR
...
would start out the deletion process of B by calling \code{cas(B.next, C, with_tag(C, 1))}. Should
this fail, $T\sb{1}$ can just read \code{B.next} again, and retry. When it succeeds, it may try to
