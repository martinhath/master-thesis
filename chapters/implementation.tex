...
for thread synchronisation. Only addresses in \code{ALLOCS} are subject for reclamation.
...
\code{Vec}, and following the pointer. Since the data is thread local, no synchronisation is needed
when operating on the \code{Vec}, which makes updates cheap.
In addition all threads store the allocations they have done since the last reclamation pass. See
\cref{sec:impl-signalvec}

One caveat of CMR is that new threads needs to register themselves before using the system. This is
done through \code{cmr::register_thread()}. This initialises thread local data, and pushes a
thread handle used in \cref{sec:impl-snapshot}. See \cref{sec:impl-thread-registration} for
information about thread registration.
...
\code{new} and \code{get_mut} are \code{unsafe} methods; \code{new} because we can not guarantee
...
pub fn guard<'a, T>(guard: &'a mut Guard<T>, a: &Atomic<T>) -> NullablePtr<'a, T> {
    without_reclamation(|| { let p = unsafe { a.load(SeqCst) };
                             ...
                             p }) }
\end{lstlisting}
Since we guarantee that no pass happend in between reading the \code{Atomic} and protecting the
...
the reclamating thread (this is why we have~\coderef{R3} in \cref{lst:reclaimer}).
In order to be more thread friendly we do not grab the global lock every time we allocate; instead
each thread flips a bit when they allocate, to signal to other threads that they might be holding
some internal lock. Upon exit they flip the bit back. When some other thread wants to make sure
that no thread is holding the lock, it flips a flag, and waits for all threads to have their bit
set to zero. This is effectively a read-write lock.
...
\code{join}. The former may be circumvented by aquiring the allocation lock before calling it, but
...
To handle this compilication we made \code{SignalVec}, a \code{Vec} that supports asynchronous
