...
allocation~\coderef{R3} (see Section~\ref{sec:alloc-lock}). This may block, but since we have not
...
signal handler. After they are initialized, we freeze all threads~\coderef{R5}, and count how many
threads were successfully frozen, which is used in~\coderef{R6} where we wait on all threads to
finish their job. When done, the data that the threads write out, their \mc{Guard}s and
...
An excerpt of the definitions of \mc{Guard} is shown in Listing~\ref{lst:guard-impl}.
\begin{figure}[t]
...
possible to construct a \mc{NullablePtr} from the \mc{Guard}.
The \code{guard!} macro corresponds to make-guard (Eq. \ref{eq:make-guard}). copy-guard
...
\begin{enumerate*}[1) ]
  ...
  and\item the user never gets a binding to a variable of type \code{Guard}, but only of type
    ...
    \mc{Guard} being non-movable from Definition~\ref{def:guard}.
\end{enumerate*}
...
the reclamating thread (this is why we have~\coderef{R3} in Listing~\ref{lst:reclaimer}). This
